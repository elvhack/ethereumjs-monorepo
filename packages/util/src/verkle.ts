...
export function getVerkleStem(
  ...
  if (typeof treeIndex === "number") {
    ...
    treeIndexBytes = setLengthRight(
      bigIntToBytes(BigInt(treeIndex), true).slice(0, 32),
      32
    );
  }

  const treeStem = verkleCrypto
    .getTreeKey(address32, treeIndexBytes, 0)
    .slice(0, 31);
...
export type VerkleLeafType =
  (typeof VerkleLeafType)[keyof typeof VerkleLeafType];
...
function powBigInt(a: any, b: any) {
  let result = 1n;
  while (b > 0n) {
    if (b % 2n === 1n) result = a;
    a = a;
...
function pow(a: any, b: any) {
  return powBigInt(BigInt(a), BigInt(b));
}

export const VERKLE_MAIN_STORAGE_OFFSET = pow(256, VERKLE_CODE_CHUNK_SIZE);
 ...
 */
export const getVerkleKey = (
  stem: Uint8Array,
  leaf: VerkleLeafType | Uint8Array
) => {
  switch (leaf) {
...
export function getVerkleTreeIndicesForCodeChunk(chunkId: number) {
  const treeIndex = Math.floor(
    (VERKLE_CODE_OFFSET + chunkId) / VERKLE_NODE_WIDTH
  );
...
export const getVerkleTreeKeyForStorageSlot = async (
  ...
  const { treeIndex, subIndex } =
    getVerkleTreeIndicesForStorageSlot(storageKey);
 ...
 */
export function decodeVerkleLeafBasicData(
...
export function encodeVerkleLeafBasicData(account: Account): Uint8Array {
  ...
  const encodedNonce = setLengthLeft(
...
export const generateCodeStems = async (
  ...
  const numStems =
    numChunks > VERKLE_CODE_OFFSET
      ? Math.ceil(numChunks / VERKLE_NODE_WIDTH) + 1
      : 1;
  const chunkStems = new Array<Uint8Array>(numStems);
  // Compute the stem for the initial set of code chunks
  chunkStems[0] = (
    await getVerkleTreeKeyForCodeChunk(address, 0, verkleCrypto)
  ).slice(0, 31);
